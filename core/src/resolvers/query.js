...
const Query = {
  ...
  // Returns a the most recent of a users' timesheet
  async userTimesheet(parent, args, ctx, info) {
    const startMonth = getStartMonth();
    return ctx.db.query.timesheets({ user: { connect: { id: ctx.request.userId } }, filter: { from_gte: startMonth } }, info);
  },
  ...
  async users(parent, args, ctx, info) {
    const self = await ctx.db.query.account(
  ...
  async userProjects(parent, args, ctx, info) {
    const projects = await ctx.db.query.projects(
      { where: { users_some: { connect: { id: ctx.request.userId } } } },
      info
    );
    return projects;
  },
  async recentWorkperiods(parent, args, ctx, info) {
    ...
    const existingTimesheetArr = await ctx.db.query.workPeriods(
      { filter: { from_gte: startWeek } },
      info
    );
  ...
  async checkinTime(parent, args, ctx, info) {
    const workPeriod = await ctx.db.query.workPeriods(
      { user: { connect: { id: ctx.request.userId } }, where: { to: null } },
      "{id from to}"
    );
    return workPeriod;
  }
